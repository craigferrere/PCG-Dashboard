...
def fetch_all_ssrn_emails():
    ...
    try:
        ...
        account = outlook.Folders["cferrere@law.harvard.edu"]
        inbox = account.Folders["Inbox"]
        ssrn_folder = inbox.Folders["ssrn"]
        messages = ssrn_folder.Items
        messages.Sort("[ReceivedTime]", True)
        for i in range(1, messages.Count + 1):
            try:
                message = messages.Item(i)
                msgclass = getattr(message, "MessageClass", "")
                subject = getattr(message, "Subject", "")
                if (
                    hasattr(message, "Body")
                    and hasattr(message, "Subject")
                    and msgclass == "IPM.Note"
                ):
                    emails.append({
                        "subject": subject,
